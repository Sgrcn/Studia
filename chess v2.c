                ...
                for(int kierunek = 0 ; kierunek < MAX_KIER[figura] ; ++kierunek)
                {
                    for(int odleglosc = 1 ; odleglosc < MAX_ODL[figura] ; ++odleglosc)
                    {
                        ...
                        if(dx < 0 || dx > 7 || dy < 0 || dy > 7 ||
                           (p[dy][dx] >= 6 && p[dy][dx] <= 11) ||
                           ((p[py][px] == 11) && ((dx==px&&p[dy][dx]!=Puste)||(dx!=px&&p[dy][dx]==Puste))) ||
                           ((p[py][px] == 11)&&(dy-py==2)&&((p[dy][dx]!=Puste)||(py!=1)))) break;
                        ...
                        if(wynik>=wmax){
                            //if(glebokosc==4)printf("dziala");
                            //printf("%d %d %d\n",wynik, kierunek, odleglosc);
                            wmax=wynik, *x=px, *y=py, *k=kierunek, *o=odleglosc;
                            //if(glebokosc==4)printf("%d %d %d %d\n", *x, *y, *k, *o);
                        }
            ...
            for(int px = 0 ; px < 8 ; ++px)
            {
                ...
                for(int kierunek = 0 ; kierunek < MAX_KIER[figura] ; ++kierunek)
                {
                    for(int odleglosc = 1 ; odleglosc < MAX_ODL[figura] ; ++odleglosc)
                    {
                        ...
                        if(dx < 0 || dx > 7 || dy < 0 || dy > 7 ||
                           p[dy][dx] < 6 ||
                           ((p[py][px] == 5) && ((dx==px&&p[dy][dx]!=Puste)||(dx!=px&&p[dy][dx]==Puste))) ||
                           ((p[py][px] == 5)&&(dy-py==-2)&&((p[dy][dx]!=Puste)||(py!=6)))) break;
